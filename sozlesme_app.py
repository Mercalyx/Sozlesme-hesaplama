...
# Oda Bilgileri
st.header("🛏️ Oda Bilgileri")
oda_bilgileri = []
if not oda_farkli_mi:
    tek_kisilik_sayisi = st.number_input("Tek Kişilik Oda Sayısı", min_value=0, value=0)
    cift_kisilik_sayisi = st.number_input("Çift Kişilik Oda Sayısı", min_value=0, value=0)
    tek_kisilik_fiyat = st.number_input("Tek Kişilik Oda Fiyatı (gecelik)", min_value=0.0, value=0.0)
    cift_kisilik_fiyat = st.number_input("Çift Kişilik Oda Fiyatı (gecelik)", min_value=0.0, value=0.0)
    for gun in range(gun_sayisi):
        oda_bilgileri.append({"tek": tek_kisilik_sayisi, "cift": cift_kisilik_sayisi, "tek_f": tek_kisilik_fiyat, "cift_f": cift_kisilik_fiyat})
else:
    for gun in range(gun_sayisi):
...
if not etkinlik_farkli_mi:
    num = st.number_input("Kaç farklı etkinlik olacak?", min_value=1, value=1, step=1)
    g_etkinlikler = []
    for i in range(num):
        st.subheader(f"Etkinlik {i+1}")
        tur = st.selectbox(f"Etkinlik Türü {i+1}", options=etkinlik_turleri, key=f"tur{i}")
        fiyat = st.number_input(f"{tur} Fiyatı (Kişi Başı)", min_value=0.0, key=f"fiyat{i}")
        kisi = st.number_input(f"{tur} Katılımcı Sayısı", min_value=0, key=f"kisi{i}")
...
for gun in etkinlikler:
    for etkinlik in gun:
        ...
        if etkinlik["tur"] == "Toplantı":
            vergi = e_tutar * 0.20
        else:
            vergi = e_tutar * 0.20  # İleride değiştirilebilir
